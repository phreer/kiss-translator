...
export const I18N = {
  ...
  system_prompt_helper_2: {
    zh: `仅在这里编写翻译原则、风格和限制条件；输入输出协议会根据模板自动附加。`,
    en: `Use this field for translation rules, tone, and constraints only. Input/output protocol details are generated automatically from the templates.`,
    zh_TW: `這裡只需編寫翻譯原則、風格與限制條件；輸入輸出協議會依模板自動附加。`,
    ja: `ここには翻訳ルール・文体・制約のみを記述してください。入出力プロトコルはテンプレートから自動生成されます。`,
    ko: `여기에는 번역 원칙, 스타일, 제약만 작성하세요. 입출력 프로토콜은 템플릿에서 자동 생성됩니다.`,
  },
  if_pre_init: {
    zh: `是否预初始化`,
