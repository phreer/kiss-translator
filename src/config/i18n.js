...
export const I18N = {
  ...
  batch_prompt_helper: {
    zh: `批量翻译输入/输出格式由所选提示词决定，可在“提示词管理”页面配置`,
    en: `Batch translation input/output format is determined by the selected prompt; configure it in the prompt management page`,
    zh_TW: `批量翻譯輸入/輸出格式由所選提示詞決定，可在「提示詞管理」頁面配置`,
    ja: `一括翻訳の入出力形式は選択したプロンプトで決まります。「プロンプト管理」ページで設定できます`,
    ko: `일괄 번역 입력/출력 형식은 선택한 프롬프트에 따라 결정되며, 프롬프트 관리 페이지에서 구성할 수 있습니다`,
    tr: `Toplu çeviri giriş/çıkış biçimi seçilen istem tarafından belirlenir; istem yönetimi sayfasında yapılandırabilirsiniz`,
  },
  io_input_format: {
    zh: `输入格式`,
    en: `Input Format`,
  ...
  io_input_template: {
    zh: `自定义输入模板`,
    en: `Custom Input Template`,
    zh_TW: `自訂輸入範本`,
    ja: `カスタム入力テンプレート`,
    ko: `사용자 정의 입력 템플릿`,
    tr: `Özel Giriş Şablonu`,
  },
  io_input_template_helper: {
    zh: `使用模板引擎语法，如 {{to_lang}} / {% for seg in segments %}，留空回退 JSON 模板`,
    en: `Uses template engine syntax, e.g. {{to_lang}} / {% for seg in segments %}; leave empty to fall back to the JSON template`,
    zh_TW: `使用模板引擎語法，如 {{to_lang}} / {% for seg in segments %}，留空回退 JSON 模板`,
    ja: `テンプレートエンジン構文を使用します（例: {{to_lang}} / {% for seg in segments %}）。空欄の場合は JSON テンプレートにフォールバック`,
    ko: `템플릿 엔진 구문을 사용합니다(예: {{to_lang}} / {% for seg in segments %}). 비워두면 JSON 템플릿으로 대체됩니다`,
    tr: `Şablon motoru sözdizimini kullanır, örn. {{to_lang}} / {% for seg in segments %}; boş bırakılırsa JSON şablonuna döner`,
  },
  batch_auto_example: {
    zh: `自动渲染的示例（请求时自动追加）`,
    en: `Auto-rendered example (appended on request)`,
    zh_TW: `自動渲染的範例（請求時自動附加）`,
    ja: `自動レンダリングされる例（リクエスト時に自動追加）`,
    ko: `자동 렌더링 예시 (요청 시 자동 추가)`,
    tr: `Otomatik oluşturulan örnek (istekte otomatik eklenecek)`,
  },
  batch_auto_example_helper: {
    zh: `此部分由所选输入/输出格式自动渲染，发送请求时自动追加到系统提示词，不可编辑`,
    en: `Rendered automatically from the selected input/output formats and appended to the system prompt when a request is sent. Read-only.`,
    zh_TW: `此部分由所選輸入/輸出格式自動渲染，傳送請求時自動附加到系統提示詞，不可編輯`,
    ja: `この部分は選択した入力/出力形式から自動レンダリングされ、リクエスト送信時にシステムプロンプトへ自動追加されます。編集できません`,
    ko: `이 부분은 선택한 입력/출력 형식에서 자동으로 렌더링되며, 요청 전송 시 시스템 프롬프트에 자동 추가됩니다. 편집할 수 없습니다`,
    tr: `Bu bölüm, seçilen giriş/çıkış biçimlerinden otomatik olarak oluşturulur ve istek gönderildiğinde sistem istemine otomatik eklenir. Düzenlenemez`,
  },
  nobatch_prompt: {
    zh: `非聚合翻译提示词`,
  ...
  prompt_test_code: {
    zh: `含代码段落`,
    en: `Paragraph with code`,
    zh_TW: `含程式碼段落`,
    ja: `コードを含む段落`,
    ko: `코드가 포함된 단락`,
    tr: `Kod içeren paragraf`,
  },
  prompt_test_multi_line: {
    zh: `多行文本`,
    en: `Multi-line text`,
    zh_TW: `多行文字`,
    ja: `複数行テキスト`,
    ko: `여러 줄 텍스트`,
    tr: `Çok satırlı metin`,
  },
  prompt_test_batch_simple: {
    zh: `简单多段落`,
    en: `Simple multi-segments`,
    zh_TW: `簡單多段落`,
    ja: `シンプルな複数セグメント`,
    ko: `간단한 다중 세그먼트`,
    tr: `Basit çoklu segment`,
  },
  prompt_test_batch_large: {
    zh: `大批量段落`,
    en: `Large batch`,
    zh_TW: `大批量段落`,
    ja: `大量バッチ`,
  ...
  prompt_test_batch_mixed: {
    zh: `混合长度段落`,
    en: `Mixed length segments`,
    zh_TW: `混合長度段落`,
    ja: `長さが混在するセグメント`,
    ko: `혼합 길이 세그먼트`,
    tr: `Karışık uzunlukta segmentler`,
  },
  prompt_test_literary: {
    zh: `文学作品`,
    en: `Literary works`,
    zh_TW: `文學作品`,
    ja: `文学作品`,
  ...
  prompt_test_batch_tech_article: {
    zh: `技术文章`,
    en: `Tech article`,
    zh_TW: `技術文章`,
    ja: `技術記事`,
  ...
  prompt_test_no_api: {
    zh: `请先配置至少一个 AI API`,
    en: `Please configure at least one AI API first`,
    zh_TW: `請先設定至少一個 AI API`,
    ja: `まず少なくとも1つの AI API を設定してください`,
    ko: `먼저 최소 하나의 AI API를 구성해 주세요`,
    tr: `Lütfen önce en az bir AI API yapılandırın`,
  },
};
