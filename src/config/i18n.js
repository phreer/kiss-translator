...
export const I18N = {
  ...
  llm_output_format_helper: {
    zh: `显式指定返回格式；修改 Prompt 后建议同步确认，避免解析走自动检测。`,
    en: `Explicitly specify the response format. After editing the prompt, confirm this setting to avoid falling back to auto-detection.`,
    zh_TW: `明確指定回傳格式；修改 Prompt 後建議同步確認，以避免解析退回自動偵測。`,
    ja: `応答形式を明示的に指定します。Prompt を編集した後は、この設定も確認して自動検出へのフォールバックを避けてください。`,
    ko: `응답 형식을 명시적으로 지정합니다. 프롬프트를 수정한 뒤에는 자동 감지로 돌아가지 않도록 이 설정도 함께 확인하세요.`,
  },
  llm_output_format_auto: {
    zh: `自动检测（兼容模式）`,
    en: `Auto Detect (Compat)`,
    zh_TW: `自動偵測（相容模式）`,
    ja: `自動検出（互換モード）`,
    ko: `자동 감지(호환 모드)`,
  },
  ...
  llm_input_segment_template: {
    zh: `LLM 输入段落模板`,
    en: `LLM Input Segment Template`,
    zh_TW: `LLM 輸入段落模板`,
    ja: `LLM 入力セグメントテンプレート`,
    ko: `LLM 입력 세그먼트 템플릿`,
  },
  llm_input_separator: {
    zh: `LLM 输入段落分隔符`,
    en: `LLM Input Segment Separator`,
    zh_TW: `LLM 輸入段落分隔符`,
    ja: `LLM 入力セグメント区切り`,
    ko: `LLM 입력 세그먼트 구분자`,
  },
  llm_output_template: {
    zh: `LLM 输出模板`,
    en: `LLM Output Template`,
    zh_TW: `LLM 輸出模板`,
    ja: `LLM 出力テンプレート`,
    ko: `LLM 출력 템플릿`,
  },
  llm_output_segment_template: {
    zh: `LLM 输出段落模板`,
    en: `LLM Output Segment Template`,
    zh_TW: `LLM 輸出段落模板`,
    ja: `LLM 出力セグメントテンプレート`,
    ko: `LLM 출력 세그먼트 템플릿`,
  },
  llm_output_separator: {
    zh: `LLM 输出段落分隔符`,
    en: `LLM Output Segment Separator`,
    zh_TW: `LLM 輸出段落分隔符`,
    ja: `LLM 出力セグメント区切り`,
    ko: `LLM 출력 세그먼트 구분자`,
  },
  llm_output_mapping_mode: {
    zh: `LLM 输出映射方式`,
    en: `LLM Output Mapping Mode`,
    zh_TW: `LLM 輸出映射方式`,
    ja: `LLM 出力マッピング方式`,
    ko: `LLM 출력 매핑 방식`,
  },
