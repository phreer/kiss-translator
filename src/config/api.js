...
Rules:
1.  **Context**: Use title/description for context only; do not output them.
2.  **HTML & Whitespace**: Preserve all HTML tags (e.g., <b>, <span>, <br>) and whitespace exactly as they appear in the structure. Only translate the text content inside them.
3.  **Glossary**: Highest priority. Follow 'glossary'. Use value for translation; if value is "", keep the key.
4.  **Do Not Translate**: Content inside <code>, <pre>, text in backticks ("code"), and placeholders like {1}, {{1}}, [1], [[1]].
5.  **Tone**: Apply the specified tone.
`;
...
// 其 normalize/denormalize 复用对应输出解析器的转义，保证 %/ 原文不被误当结构字符。
// 输入/输出格式预设各自携带完整编解码器，运行时按所选组合叠加或去重。
export const inputFormatPresets = {
  json: {
    name: "json",
    inputTemplate: JSON_INPUT_TEMPLATE,
    normalize: getParserPreset("json").normalize,
    denormalize: getParserPreset("json").denormalize,
  },
  percent: {
    name: "percent",
    inputTemplate: PERCENT_INPUT_TEMPLATE,
    normalize: getParserPreset("percent").normalize,
    denormalize: getParserPreset("percent").denormalize,
  },
};

export const getInputFormatPreset = (name) => {
  return inputFormatPresets[name] || inputFormatPresets.json;
};

// 聚合输入/输出格式由所选批处理提示词（config/prompt.js）携带，
// 运行时经 resolveApiPromptSettings 内联为 apiSetting.ioInputFormat/ioOutputFormat。
// 这里仅负责把格式名解析为具体的输入/输出预设。
export const resolveIoPreset = (inputFormat = "json", outputFormat = "json") => ({
  inputFormat: getInputFormatPreset(inputFormat),
  outputFormat: getParserPreset(outputFormat),
});

/**
 * 合并输入/输出格式的编解码器，供请求构建与结果解析共用同一转义规则。
 *
 * normalize 先施加输出格式转义（译文若被模型原样回显，输出解析器可还原），再施加
 * 输入格式转义（保护输入结构字符）；denormalize 按逆序撤销：先还原输入格式，再还原
 * 输出格式。两者为同一格式（name 相同）时只执行一次，避免重复转义/双重解码。
 *
 * @param {Object} inputFormat 输入格式预设（携带 normalize/denormalize/name）
 * @param {Object} outputFormat 输出格式解析器预设（携带 normalize/denormalize/name）
 * @param {"normalize"|"denormalize"} direction 处理方向
 * @returns {Function} 应用编解码的纯函数
 */
export const combineIoCodec = (inputFormat, outputFormat, direction) => {
  const same = inputFormat?.name === outputFormat?.name;
  if (direction === "normalize") {
    return (value) => {
      const text = outputFormat.normalize(String(value ?? ""));
      return same ? text : inputFormat.normalize(text);
    };
  }
  return (value) => {
    const text = inputFormat.denormalize(String(value ?? ""));
    return same ? text : outputFormat.denormalize(text);
  };
};
