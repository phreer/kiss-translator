 ...
 * @param {Object|null} inputFormat 输入格式 preset，与 parser 组合还原输入侧转义（同一格式时自动去重）
 * @returns {Array<[string, string]>} 解析后的双元组列表 [译文, 源语言检测结果]
 */
const parseAIRes = (raw, useBatchFetch = true, parser = null, inputFormat = null) => {
  if (!raw) {
    return [];
  }
  ...
  const decodeText = parser
    ? inputFormat
      ? combineIoCodec(inputFormat, parser, "denormalize")
      : parser.denormalize
    : decodeHTMLEntities;
...
export async function* handleTranslate(
  ...
  // 供流式 denormalize 与兜底解析共用；输入格式预设用于还原输入侧转义。
  ...
  const batchFormat = outputFormat.name;
...
async function* handleTranslateStreamInternal(
  ...
  const decodeText = ioInputFormat
    ? combineIoCodec(ioInputFormat, getParserPreset(batchFormat), "denormalize")
    : getParserPreset(batchFormat).denormalize;
