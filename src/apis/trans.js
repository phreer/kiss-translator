...
export const genTransReq = async ({ reqHook, ...args }) => {
  ...
  if (API_SPE_TYPES.ai.has(apiType)) {
    ...
    args.systemPrompt = useBatchMode
      ? `${baseSystemPrompt}\n\n${buildBatchExample(inputFormat, outputFormat)}`
      : baseSystemPrompt;
...
export async function* handleTranslate(
  ...
  const { inputFormat, outputFormat } = apiSetting.useBatchFetch
    ? resolveIoPreset(apiSetting.ioInputFormat, apiSetting.ioOutputFormat)
    : {
        inputFormat: getInputFormatPreset("json"),
        outputFormat: getParserPreset("json"),
      };
...
async function* handleTranslateStreamInternal(
  ...
  // 输出格式由所选批处理提示词 preset（ioOutputFormat）固定。与非流式 parseAIRes
  // 一样按 preset 驱动解析器选择，不再对响应内容做字节嗅探（detectStreamFormat）。
  const outputFormatName = ioOutputFormat?.name || "json";
  const isJsonFormat = outputFormatName === "json";
  const isPercentFormat = outputFormatName === "percent";
  const jsonParser = createStreamingJsonParser();
  const realtimeParser =
    streamRenderMode === "realtime"
      ? createRealtimeStreamParser({ format: outputFormatName })
      : null;
    ...
    for await (const rawData of fetchStream(input, init, {
      ...
      try {
        ...
        if (delta) {
          ...
          if (isJsonFormat) {
            // JSON 由增量解析器逐 delta 抽取已闭合对象，解析器内部维护未完成状态。
            ...
            for (const { id, translation } of parseStreamingPercentSegments(
              fullContent,
              processedIds,
              { decodeText }
            )) {
            ...
            // xml / textlines：增量解析已闭合的段落，未完成尾部留到后续 delta 或结尾兜底。
          ...
          if (realtimeParser && streamRenderMode === "realtime") {
            ...
            for (const { id, partialText, isComplete } of items) {
              if (!isComplete) {
                yield {
                  id,
                  partialText: decodeText(partialText),
                  isComplete: false,
                };
              }
            }
